...
export default function CodeViewer({ card }) {
  ...
  useEffect(() => {
    ...
    const js = card.js ? `<script>window.addEventListener('DOMContentLoaded',()=>{${card.js}})</script>` : ''
    ...
    <div className="bg-slate-900 text-slate-100 rounded-lg p-4 h-full flex flex-col cv-fade">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <small className="text-slate-400">{card.language ?? 'Animation'}</small>
        </div>

          <div className="flex items-center gap-2">
          <div className="flex rounded-md bg-white/3 p-1">
            <button onClick={() => setTab('preview')} className={`px-3 py-1 text-sm rounded ${tab === 'preview' ? 'bg-indigo-600 text-white' : 'text-slate-200/80 hover:bg-white/5'}`}>
              Preview
            </button>
            <button onClick={() => setTab('code')} className={`ml-3 px-3 py-1 text-sm rounded ${tab === 'code' ? 'bg-indigo-600 text-white' : 'text-slate-200/80 hover:bg-white/5'}`}>
              Code
            </button>
          </div>
          <button onClick={copy} className={`px-3 py-1 bg-white/6 hover:bg-white/8 text-sm rounded ${copied ? 'copy-anim' : ''}`}>
            {copied ? 'Copied' : 'Copy'}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1">
        {tab === 'preview' ? (
          ...
          <div className={`terminal bg-slate-900 rounded-lg overflow-hidden ${tab === 'code' ? 'terminal-glow' : ''}`}>
            ...
            <pre className="p-4 overflow-auto text-sm text-slate-100 text-left font-mono code-flicker"><code className="whitespace-pre">{prettyCode()}</code></pre>
          </div>
        )}
      </div>
    </div>
